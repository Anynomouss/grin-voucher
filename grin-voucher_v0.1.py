...
@author: Anynomous
Note that this tool must be run in a secure environment since vouchers are by default not encrypted with a password
"""
...
from grinmw.wallet_v3 import WalletV3 # pip install grinmw
import pprint, os
...
if __name__ == '__main__':
    ###############################################################################
    ## 1) Parse command line arguments using arparse
    parser = argparse.ArgumentParser(description = "Grin-voucher command line parsse using argparse")
    ...
    if argument.nvouchers:
        ...
        status = True
    if argument.end:
        print("You have used '-v' or '--value' with argument: {0}".format(argument.value))
        status = True
    ...
    if argument.password:
        ...
        status = True  
    if not status:
        print("Maybe you want to use -H --Help or -n --nvouchers -v --value,  for example running 'grin-voucher.py -n 1 -v 1.0', creates a single voucher with a value of 1") 
    
    ## 2)  Check the input arguments, if they are empty, set booleans so that we will not test for them
    ## Do some basic logic checks to prevent users from making invalid or malicious vouchers
    ...
    ###############################################################################
    ## 2) Crate X number of temporary wallets using Mimblewimble-py, store their data
    ## as json | mnemonic | QR
    ###############################################################################
